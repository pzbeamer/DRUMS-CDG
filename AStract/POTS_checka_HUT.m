...
for pt=3:872
    ...
    if isfile(strcat('/Volumes/GoogleDrive/Shared drives/REU shared/LSA/HUT/',T{pt,1}{1},'_HUT_WS.mat'))
        ...
        end_avg_ind=find(abs(Tdata-(HUT_start-10))==min(abs(Tdata-(HUT_start-10))));
        if betweenTimes(1,pt-2)<40
            begin_avg_ind=1;
        else
            begin_avg_ind=find(abs(Tdata-(HUT_start-40))==min(abs(Tdata-(HUT_start-40))));
        end

...
for pt = 6%[7:17 19:34 36:71 73:106 108:125 127:161 163:173 175:209] %211:872]%[3:17 19:818 820:872]
    pt
    pt_id = T{pt,1}{1}
...
% 
                 ...
                 markers(pt-3,13) = mean(Hdata(1:round(val_start))); %HRbeforeVal
                 markers(pt-3,14) = mean(Hdata(round(val_end):end)); %HRafterVal
                 markers(pt-3,15) = mean(Pdata(1:round(val_start))); %SBPbeforeVal
                 markers(pt-3,16) = mean(Pdata(round(val_end):end)); %SBPafterVal
% 
%         %% Max HR and BP
% 
%         %Phase 2
...
%                 markers(pt-3,10) = max(Hdata(data.i_te:data.i_t3)); %maxHR3
            ...
            markers(pt-3,4)=values(1); %SP max phase 1
            markers(pt-3,5)=values(2); %SP at end on phase 1
            markers(pt-3,6)=values(3); %SP min phase 2
            markers(pt-3,7)=values(4); %SP max phase 2
            markers(pt-3,8)=values(5); %SP min phase 4
            markers(pt-3,9)=values(6); %H max phase 3
            markers(pt-3,10)=values(7); %H min phase 4
    end 
end
%% Save    

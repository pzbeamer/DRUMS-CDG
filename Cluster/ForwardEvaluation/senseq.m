...
function [sens, y, sol] = senseq(pars,data) % wrapper

[sol,y] = model_sol(pars,data); %evaluates at nominal paramenters

%Depending on form of y, the sensitivity matrix could be dymodel/dpars or 
%dr/dpars where r = ymodel - ydata

[sens] = diffjac(pars,@myfun,y,data);


function y = myfun(pars,data)
 
[~,y] = model_sol(pars,data); %evaluates at the change in paras
...
%          f0   = f(x), preevaluated
%x = x(1:end-1); %Subtract 1 to not do sensitivity analysis on delay parameter
n = length(x);  
jac = zeros(length(f0), n);
for j = 1:n
    zz = zeros(n,1);
    zz(j) = 1;
    jac(:,j) = dirder(x,zz,f,f0,data); % f- myfun and f0- initial model
end
...
del = x+epsnew*w;
f1  = feval(f,del,data); 
z   = (f1 - f0)/epsnew; %forward difference

function Func_DriverBasic_c(pt_file_name,restTime,INDMAP,minPeakDistance,sampleRate)
    ...
    s = (1:sampleRate:length(val_dat(:,1)))'; %Sampling vector
    sdat = val_dat(s,:);
    Tdata = sdat(:,1);
    ECG = sdat(:,2);
    Hdata = sdat(:,3);
    ...
    [SPdata S] = SBPcalc_ben(val_dat(:,1),val_dat(:,4),minPeakDistance,0);
    SPdata = SPdata(s);
    ...
    clear i
    clear pt_file_name
    save(strcat('Valsalva/nomHR_residuals/',pt(1:(end-7)),'_',num2str(restTimeS),'_',num2str(sampleRate),'_nomHR.mat'))

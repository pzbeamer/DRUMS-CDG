...
u=[37 48 59 60 65 66 67 76 85 102];

for k = 1:10
    j = 10;
    %optimizing
    pt_name = strcat(T{u(j),1}{1},'_val1_WS.mat'); 
    %{
...
%     INDMAP = [2 6 7 8 19 20 21];
    ...
    for i = 0%:3
        ...
        Func_DriverBasic_c(pt_name,[30 - 5*i 30],INDMAP,T{u(j),89},srate);
    
        nomHRfile =strcat('Valsalva/nomHR_residuals/',pt_name(1:end-7),'_',num2str(30-5*i),'_',num2str(srate),'_nomHR.mat');
        load(nomHRfile)
        %Call optimization with file generated by forward evaluation
        HR_LM = Func_DriverBasic_LM_c(nomHRfile,INDMAP,k,srate);
    ...
    %% plots
        ...
        if i == 4
            ...
            load(strcat('Valsalva/nomHR_residuals/',T{u(j),1}{1},'_Val1_',num2str(30 - 5*num),'_nomHR.mat'))
            load(strcat('Valsalva/optHR_residuals/',T{u(j),1}{1},'_Val1_',num2str(30 - 5*num),'_',num2str(srate),'_',num2str(k),'_optHR.mat'))
        else
            load(strcat('Valsalva/nomHR_residuals/',T{u(j),1}{1},'_Val1_',num2str(30 - 5*i),'_',num2str(srate),'_nomHR.mat'))
            load(strcat('Valsalva/optHR_residuals/',T{u(j),1}{1},'_Val1_',num2str(30 - 5*i),'_',num2str(srate),'_',num2str(k),'_optHR.mat'))
        end
